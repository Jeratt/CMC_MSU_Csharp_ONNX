    ...
    public class ModelManager //ONNX model manager
    {
        ...
        public void Init()
        {
            if (session is null) {
                ...
                if (tolerance == 4)
                    throw new Exception("Failed to download a model");
                else
                    session = new InferenceSession("tinyyolo2-8.onnx");
            }
        }

        public Task<List<ObjectBox>> PredictAsync(Image<Rgb24> img, CancellationToken ct, string prefix="")
        {
        ...
        public void LoadModel()
        {
            using (WebClient client = new WebClient())
            {
                if (!this.fileManager.CheckIfExists("tinyyolo2-8.onnx"))
                {
                    ...
                    client.DownloadFile(this.ModelPath, "tinyyolo2-8.onnx");
